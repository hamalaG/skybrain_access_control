<x-app-layout>
	...
	<div class=" flex items-center justify-between pb-6">
		<div>
			<h2 class="text-gray-600 font-semibold">Authorized Vehicles</h2>
		...
		<div class="flex items-center justify-between">
			<div class="flex bg-gray-100 items-center p-2 rounded-md">
				...
				<div class="lg:ml-40 ml-10 space-x-8">
                    ...
                    <a href="authorizedvehicles/create">
                        <button class="bg-indigo-600 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer">New Vehicle</button>
                    </a>
                </div>
			</div>
		</div>
			...
			<div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
				<div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
					<table class="min-w-full leading-normal">
                        ...
                        @foreach ($authorizedvehicles as $authorizedvehicle)
						<tbody>
							<tr>
								...
								<td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        {{$authorizedvehicle->plate_number}}
                                    </p>
								</td>
								<td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
									<p class="text-gray-900 whitespace-no-wrap">{{$authorizedvehicle->vehicle_type}}</p>
								</td>
								<td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
									<p class="text-gray-900 whitespace-no-wrap">
										{{$authorizedvehicle->vehicle_model}}
									</p>
								</td>
								<td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
									<p class="text-gray-900 whitespace-no-wrap">
										{{$authorizedvehicle->vehicle_owner}}
									</p>
								</td>
								<td class="flex px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <a href="{{ route('authorizedvehicles.show',$authorizedvehicle->id) }}">
                                    ...
                                    <a href="{{ route('authorizedvehicles.edit',$authorizedvehicle->id) }}">
                                    ...
                                    <form method="POST" action="{{ route('authorizedvehicles.destroy',$authorizedvehicle->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <a href="{{ route('authorizedvehicles.destroy',$authorizedvehicle->id) }}" onclick="event.preventDefault(); this.closest('form').submit();">
