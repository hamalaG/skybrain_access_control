<x-app-layout>
	...
	<div class=" flex items-center justify-between">
		<div>
			<h2 class="text-gray-600 font-semibold">Recognized People</h2>
			<span class="text-xs">Showing Daily Recognized People</span>
		</div>
		<div class="flex items-center justify-between">
					...
					<table class="min-w-full leading-normal">
						...
						<tbody>
							<tr>
								...
								<td class="px-5 py-3 border-b border-gray-200 bg-white text-sm">
									<p class="text-gray-900 whitespace-no-wrap">{{$person->name}}</p>
								</td>
								<td class="px-5 py-3 border-b border-gray-200 bg-white text-sm">
									<p class="text-gray-900 whitespace-no-wrap">
										{{$person->time}}
								...
								<td class="flex px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <a href="{{ route('recognizedperson.show',$person->id) }}">
                                    ...
                                    <form method="POST" action="{{ route('recognizedperson.destroy',$person->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <span class="relative inline-block px-2 py-1 font-semibold text-red-600 leading-tight">
                                            <a href="{{ route('recognizedperson.destroy',$person->id) }}" onclick="event.preventDefault(); this.closest('form').submit();">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
